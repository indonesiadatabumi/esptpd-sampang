...
class User extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('layoutbackend', 'admin/user_esptpd', $data);
    }
    ...
    public function insert()
    {
        ...
        if ($cek->num_rows() > 0) {
            ...
            $_userid = $this->Mod_user->get_userid();
            $level = $this->input->post('level');
            $result = $this->Mod_user->get_wp_wr($username);
            // var_dump($result);
            $jenis = $result->jenis;
            $golongan = $result->golongan;
            $npwprd = $result->npwprd;
            $npwpd = $jenis . '.' . $golongan . '.' . $npwprd;
            $id_wp_wr = $result->wp_wr_id;
            ...
            $this->Mod_user->insertUser("wp_user", $save);
    ...
    public function delete()
    {
        ...
        $this->Mod_user->deleteUsers($id, 'wp_user');
        $data['status'] = TRUE;
        echo json_encode($data);
    }
    ...
    function get_namawp()
    {
        ...
        $result = $this->Mod_user->get_namawp($npwp);
    ...
    function get_wp_wr()
    {
        ...
        $result = $this->Mod_user->get_wp_wr($npwp);
        $result = ($result != '') ? $result : 'NPWPD salah /Tidak Terdaftar';
        echo json_encode($result);
    }
}
